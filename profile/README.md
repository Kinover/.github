...
- **모바일 UX 개선**: 바텀시트 하단 safe-area, Android 실기기 터치/레이아웃 이슈 대응
- **전화번호 인증**: Firebase Phone Auth + 서버 idToken 검증으로 본인 확인 및 중복 번호 처리
- **다크 모드 지원**: 테마 토큰 도입 및 전역 UI·네이티브 설정 반영
- **감정 버블 UI**: 홈 화면 가족 멤버 탭 시 감정 이모지 버블 표시
- **가족 없이 홈 접근 허용**: 가족 미설정 상태에서도 홈 화면 진입 가능하도록 개선
...
- **감정 공유**: 이모지 기반 오늘의 기분 공유 및 감정 버블 UI
